<div class="bg-slate-700 w-12/12 rounded-md mx-auto flex flex-col justify-center items-center p-5">
  <h2 class=" w-fit">Would you try to say 'Hello World?'</h2>
  <div class="w-full flex flex-row justify-center items-center dark:text-slate-700">
      <div class="bg-green-300 hover:bg-green-400 h-24 w-6/12 cursor-pointer rounded-l-lg flex justify-center items-center">
        <h3>YES</h3>
      </div>
      <div class="bg-red-300 hover:bg-red-400 h-24 w-6/12 cursor-pointer rounded-r-lg flex justify-center items-center">
        <h3>NO</h3>
      </div>
  </div>
</div>



      <!-- <div class="
        bg-gradient-to-br from-DF-blue via-DF-red to-DF-yellow
        
        font-semibold 
        p-0.5 
        my-10
        md:mx-0
        rounded-xl
        cursor-pointer
      ">
        <button class="trans-300 p-2 px-8 text-DF-navy bg-slate-800/80 w-24 h-24 rounded-xl hover:bg-green-200/90" on:click>
          YES
        </button>
      </div>
      <div class="
        bg-gradient-to-br from-DF-blue via-DF-red to-DF-yellow
        
        font-semibold 
        p-0.5 
        my-10
        md:mx-0
        rounded-xl
        cursor-pointer
      ">
        <button class="trans-300 p-2 px-8 text-DF-navy bg-slate-800/80 w-24 h-24 rounded-xl hover:bg-red-200/90" on:click>
          NO
        </button>
      </div> -->