      ...
      + label and for each for reoccuring code -->
			<div class="md:w-5/12 flex flex-col bg-slate-700/20 p-2 rounded-md gap-2">
				...
				<button
					...
					on:click={() => (genderValue = 'all')}>Everyone</button
				>
				...
				<div class="grid grid-rows-2 grid-cols-2 gap-2">
					<button
						...
						on:click={() => (genderValue = 'male')}>Male</button
					>

					<button
						class="subtleBtn"
						class:active={genderValue === 'female'}
						on:click={() => (genderValue = 'female')}>Female</button
					>

					<button
						class="subtleBtn"
						class:active={genderValue === 'queer'}
						on:click={() => (genderValue = 'queer')}>Queer</button
					>

					<button
						class="subtleBtn"
						class:active={genderValue === 'other'}
						on:click={() => (genderValue = 'other')}>Other</button
					>
				</div>
			</div>
		</div>
