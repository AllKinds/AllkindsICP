...
export type ResultAnswer = { ok: Answer } | { err: Error };
export type ResultFriends = { ok: Array<[UserMatch, FriendStatus]> } | { err: Error };
export type ResultQuestion = { ok: Question__1 } | { err: Error };
export type ResultSkip = { ok: Skip } | { err: Error };
export type ResultUser = { ok: User } | { err: Error };
export type ResultUserMatch = { ok: UserMatch } | { err: Error };
export interface Skip {
    question: bigint;
    reason: { flag: null } | { skip: null };
}
...
export interface _SERVICE {
    ...
    findMatch: ActorMethod<[number, number, [] | [Gender], number], ResultUserMatch>;
